...
const Test = () => {
  ...
  useEffect(() => {
    ...
    const assignStreamToVideo = () => {
      if (videoRef.current && stream) {
        ...
        retryCount++;
        console.warn(`Retrying to assign stream to video... Attempt ${retryCount}`);
        setTimeout(assignStreamToVideo, 100); // Retry after 100ms
      } else {
        console.error('Failed to assign stream to video after multiple retries');
      }
    };
  ...
  const submitTest = async () => {
    const finalScore = Number(calculateScore()); // Ensure this is correctly calculating the score
    setScore(finalScore);
    const token = localStorage.getItem('token');
    const email = token ? getEmailFromToken(token) : null;
  
    console.log('Final Score:', finalScore); // Log the score to verify it
    console.log('User Email:', email); // Log the email for verification
    ...
    try {
      ...
      alert('An error occurred while submitting your test. You must attempt alteast one Ques.');
    }
  };
